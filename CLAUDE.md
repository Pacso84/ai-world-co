...
- **A csak kézzel gyártott mintán futó teszt az ALAKOT ellenőrzi, nem a valóságot** (2026-08-24). A `core/reel-post.js` első változatához 16 zöld teszt tartozott, közben olyan bemeneti alakot várt, ami **mind a 358 valódi útmutató közül egyre sem illett** — a minta a fejemből jött, nem a lemezről. Azóta minden ilyen modul kap egy záró esetet, ami **a `content/articles/` valódi fájljain fut végig** (lásd `core/frontmatter.test.js`, `core/reel-post.test.js`). Ugyanígy: ha egy teszt csak a válasz KULCSAIT nézi, nem az értékeket, akkor a `caption: video` elgépelés átmegy rajta.
